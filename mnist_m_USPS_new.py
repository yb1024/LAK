...
import numpy as np
from mnist_m_USPS_newtest import test
import coral
...
if __name__ == "__main__":
    ...
    #     transform=img_transform_target
    # )
    train_list = os.path.join(target_image_root, 'USPS_train_labels.txt')

    dataset_target = GetLoader(
        data_root=os.path.join(target_image_root, 'USPS_train'),
        data_list=train_list,
        transform=img_transform_target
    )
    ...
    adversarial_train_list = os.path.join(target_image_root, 'USPS_adversarial_train_labels.txt')

    adversarial_dataset_target = GetLoader(
        data_root=os.path.join(target_image_root, 'USPS_adversarial_train'),
    ...
    for epoch in range(n_epoch):
        ...
        torch.save(my_net, '{0}/mnist_m_USPS_model_epoch_{1}.pth'.format(model_root, epoch))
