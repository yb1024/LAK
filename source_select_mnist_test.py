...
def test(dataset_name, epoch):
    assert dataset_name in ['SVHN', 'mnist_m', 'MNIST', 'USPS']
    ...
    my_net = torch.load(os.path.join(
        model_root, 'mutilsource_mnist_model_epoch_' + str(epoch) + '.pth'
    ))
    my_net = my_net.eval()
    ...
    while i < len_dataloader:
        ...
        if dataset_name=='MNIST':
            ...
            n_total += batch_size
            # if (epoch % 10 == 0 or epoch == 99):
            #     temp = my_net.VisuializeT(Tinput_data=input_img)
    ...
    #     plt.show()
    #
    # if (dataset_name=='SVHN' and epoch % 10==0) or epoch==99:
