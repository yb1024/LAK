...
import numpy as np
from new_mnist_m_USPStest import test
import coral
...
if __name__ == "__main__":
    ...
    train_list = os.path.join(source_image_root, 'USPS_train_labels.txt')
    ...
    for epoch in range(n_epoch):
        ...
        while i < len_dataloader - 1:
            ...
            for j, label in enumerate(adversarial_label_temp):
                ...
                if eval(temp) == []:
                    ...
                    a = np.concatenate((eval(temp), adversarial_img_temp[j].reshape(1, 3, 28, 28)), 0)
                    temp = 'input_adversarial_s_' + str(label) + '=a'
                    exec(temp)
            ...
            if label_adversarial_s_0.shape[0] == 0 or label_adversarial_s_1.shape[0] == 0 or \
                    label_adversarial_s_2.shape[0] == 0 or label_adversarial_s_3.shape[0] == 0 or \
                    label_adversarial_s_4.shape[0] == 0 or label_adversarial_s_5.shape[0] == 0 or \
                    label_adversarial_s_6.shape[0] == 0 or label_adversarial_s_7.shape[0] == 0 or \
                    label_adversarial_s_8.shape[0] == 0 or label_adversarial_s_9.shape[0] == 0:
                i += 1
                continue
            ...
            adversarial_output9 = my_net(input_imgs, input_imgt, input_adversarial_s_0, input_adversarial_s_1,
                                         input_adversarial_s_2, input_adversarial_s_3
                                         , input_adversarial_s_4, input_adversarial_s_5, input_adversarial_s_6,
                                         input_adversarial_s_7, input_adversarial_s_8,
                                         input_adversarial_s_9, input_adversarial_t_0, input_adversarial_t_1,
                                         input_adversarial_t_2, input_adversarial_t_3,
                                         input_adversarial_t_4, input_adversarial_t_5, input_adversarial_t_6,
                                         input_adversarial_t_7, input_adversarial_t_8,
            ...
            # 加MMD
            if epoch < 50 or epoch >= 200:
                err = err_domain + err_s_label + my_net.MMD(input_imgs, input_imgt) + my_net.deepcoral(input_imgs,
                                                                                                       input_imgt)
            elif (epoch >= 50) and (epoch < 100):
                err = err_domain + 0.01 * err_adversarial + err_s_label + my_net.MMD(input_imgs,
                                                                                     input_imgt) + my_net.deepcoral(
                    input_imgs, input_imgt)
        ...
        torch.save(my_net, '{0}/USPS_mnistm_model_epoch_{1}.pth'.format(model_root, epoch))
