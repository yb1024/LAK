...
if __name__ == "__main__":
    ...
    train_list = os.path.join(source_image_root, 'SVHN_train_labels.txt')

    dataset_source = GetLoader(
        data_root=os.path.join(source_image_root, 'SVHN_train'),
    ...
    for epoch in range(n_epoch):
        ...
        while i < len_dataloader - 1:
            ...
            # 加MMD
            err = err_domain + err_s_label +2*my_net.MMD(input_imgs, input_imgt) + 0.5 * my_net.deepcoral(input_imgs,
                                                                                                         input_imgt)
            # DANN
        ...
        torch.save(my_net, '{0}/SVHN_USPS_model_epoch_{1}.pth'.format(model_root, epoch))
